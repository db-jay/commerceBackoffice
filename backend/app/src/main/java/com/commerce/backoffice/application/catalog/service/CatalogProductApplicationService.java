package com.commerce.backoffice.application.catalog.service;
...
public class CatalogProductApplicationService implements CatalogProductUseCase {
    ...
    @Override
    public Product update(long productId, UpdateCatalogProductCommand command) {
        Product product = catalogProductPersistencePort.findById(productId)
            .orElseThrow(() -> new BusinessException(ErrorCode.PRODUCT_NOT_FOUND));

        product.rename(command.name());
        product.changePrice(command.price());
        catalogProductPersistencePort.updateBasicInfo(product.id(), product.name(), product.price());
        return product;
    }

    @Override
    public Product changeStatus(long productId, ChangeCatalogProductStatusCommand command) {
        Product product = catalogProductPersistencePort.findById(productId)
            .orElseThrow(() -> new BusinessException(ErrorCode.PRODUCT_NOT_FOUND));

        ProductStatus targetStatus = ProductStatus.valueOf(command.status());
        if (targetStatus == ProductStatus.INACTIVE) {
            product.deactivate();
        } else {
            product.activate();
        }

        catalogProductPersistencePort.updateStatus(product.id(), product.status());
        return product;
    }
