...
public class Order {
    ...
    public static Order restore(Long id, Long memberId, List<OrderLine> orderLines, OrderStatus status) {
        /*
         * DB에서 조회한 주문을 "현재 상태 그대로" 복원할 때 사용한다.
         * create()는 항상 CREATED 상태로 시작하므로 조회 복원 용도와 분리한다.
         */
        Order order = new Order(id, memberId, orderLines);
        if (status != null) {
            order.status = status;
        }
        return order;
