...
### 처리(구현)
- delivery 컨텍스트 1차 구현
- 출고 준비/출고 완료/배송 중/배송 완료 같은 기본 배송 상태 전이 규칙 정의
- 송장 번호 등록과 배송 조회 흐름의 최소 API 추가

### 학습(숙지 문서)
- `docs/issues/week6/11-learning-delivery-context.md`

## 20. W6 최소 완료선
- 처리 이슈 1개 완료(PR 또는 검증 완료 기준)
- 배송 상태 전이 성공/실패, 송장 등록, 잘못된 상태 변경 테스트가 통과한다.
- `./gradlew test`, `./gradlew check` 최소 1회 성공

## 21. W6에서 꼭 구분할 것
- 주문(order)과 배송(delivery)은 연결되지만 같은 책임이 아니다.
- 배송 상태 전이는 단순 문자열 변경이 아니라 운영 단계의 의미를 가진다.
- 송장 등록은 "값 저장"이 아니라 출고 이후 흐름을 여는 운영 이벤트다.
...
- v2.4 (2026-04-05): W5 완료 상태 반영 및 W6 delivery 실행 항목 추가
